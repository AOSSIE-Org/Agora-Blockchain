...
contract OneVote {
    ...
    function getAllVotingProcesses() public view returns (address[] memory) {
        address[] memory processAddresses = new address[](processCounter);
        for (uint i = 1; i <= processCounter; i++) {
            processAddresses[i - 1] = address(votingProcesses[i]);
        }
