...
contract _Election {
    ...
    function addCandidate(string memory _name, string memory _about) public {
        ...
        candidates.push(_Candidate(candidatesCount, _name, _about, 0));
        candidatesCount++;
    }
    ...
    // Get winner details
    function getWinnerDetails() public view returns (_Candidate[] memory) {
        require(block.timestamp > electionInfo.edate, "Results can only be declared after election ends.");
        _Candidate[] memory winnerDetails = new _Candidate[](candidates.length);
        uint maxVote = 0;
        uint winnerCount = 0;
        for(uint i = 0; i < candidatesCount; i++) {
            if(candidates[i].voteCount > maxVote) {
                ...
                winnerDetails = new _Candidate[](candidates.length);
                winnerCount = 0;
